...
struct Stats { int score; int pelletsEaten; };
struct Entity { 
    Position pos; 
    sf::Color color; 
    Stats gameplayStats; 
...
void loadData() {
    ...
    if (fin) {
        fin.read(reinterpret_cast<char*>(highScores), sizeof(highScores));
        fin.close();
    } else {
        for(int i = 0; i < 20; i++) {
            highScores[i].score = 0;
            snprintf(highScores[i].playerName, sizeof(highScores[i].playerName), "PAC");
        }
    }
}
...
void saveData() {
    ...
    if (fout) {
        fout.write(reinterpret_cast<char*>(highScores), sizeof(highScores));
        fout.close();
    }
}
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLS; j++) {
            if (i == 0 || i == 24 || j == 0 || j == 24 || 
                ...
                (i == 21 && j >= 18 && j <= 21)) 
            { maze[i][j] = 1; } 
            else { maze[i][j] = 0; total++; }
        }
    }
}

// =============================================================================
// PIXEL FONT RENDERING (Restored: Q and others)
// =============================================================================
void drawPixelLetter(sf::RenderWindow& win, char c, float x, float y, float sz, sf::Color col) {
    ...
    else if (c == 'Q') { for(int j=1; j<4; j++) { d(0,j); d(2,j); } d(1,0); d(1,4); d(2,4); d(2,3); } // Restored Q
