...
void initMaze(int& total) 
{
    ...
    for (int i = 0; i < ROWS; i++) 
    {
        for (int j = 0; j < COLS; j++) 
        {
            ...
            // wall
            if (i == 0 || i == 24 || j == 0 || j == 24 || (i == 3 && j >= 3 && j <= 8) || (i == 3 && j >= 16 && j <= 21) ||(i == 4 && j >= 3 && j <= 8) || (i == 4 && j >= 16 && j <= 21) ||
                (i >= 8 && i <= 16 && j == 4) || (i == 12 && j >= 5 && j <= 20) || (i >= 8 && i <= 16 && j == 20) || (i >= 16 && i <= 21 && j == 8) || (i == 21 && j >= 9 && j <= 15) || (i == 8 && j >= 8 && j <= 16) ||
                (i >= 9 && i <= 13 && j == 12) || (i == 20 && j >= 3 && j <= 6) || (i == 21 && j >= 3 && j <= 6) || (i == 20 && j >= 18 && j <= 21) || (i == 21 && j >= 18 && j <= 21)) 
            { 
                maze[i][j] = 1; 
            } 
            else // pellet
...
void drawPixelLetter(sf::RenderWindow& win, char c, float x, float y, float sz, sf::Color col) 
{
    ...
    auto d = [&](int i, int j) 
    { 
        p.setPosition(sf::Vector2f(x + i * sz, y + j * sz)); 
        win.draw(p); 
    };
...
int main() 
{
    ...
    if (deathBuffer.loadFromFile("pacman_death.wav")) 
    ...
    while (window.isOpen()) 
    {
        while (const std::optional event = window.pollEvent()) 
        {
            ...
            if (state != PLAYING && event->is<sf::Event::KeyPressed>()) 
            {
                ...
                if (k->code == sf::Keyboard::Key::W || k->code == sf::Keyboard::Key::Up) 
                    menuSelect = 0;
                else if (k->code == sf::Keyboard::Key::S || k->code == sf::Keyboard::Key::Down) 
                    menuSelect = 1;
        ...
        if (state == PLAYING) 
        {
            ...
            if (maze[game.player.pos.r + dr][game.player.pos.c + dc] != 1) 
            {
                ...
                if (maze[game.player.pos.r][game.player.pos.c] == 0) 
                {
                    ...
                    if (game.player.gameplayStats.pelletsEaten >= game.totalPellets) 
                        state = WIN;
                }
            }
